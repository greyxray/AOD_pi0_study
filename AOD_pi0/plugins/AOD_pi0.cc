    ...
    if (true && V0Ks.isValid())
    {
      ...
      if (v0_count > 0)
      {
        ...
        for(unsigned k0_i = 0; k0_i < V0Ks->size(); k0_i++) // over V0 K0s
        {
          ...
          h_Ks_v0_pions_dR->Fill((v0_ks_pion1).DeltaR(v0_ks_pion2));
          ...
          // Possitioning
            ...
            h_Ks_v0_vz->Fill(v_Ks.Z());
            h_Ks_v0_dx->Fill(abs(v_Ks.X() - pv_position.x()));
            h_Ks_v0_dy->Fill(abs(v_Ks.Y() - pv_position.y()));
            h_Ks_v0_dz->Fill(abs(v_Ks.Z() - pv_position.z()));
          ...
          // Basic plots for Ks V0 collection
            ...
            // Loop over daughters of V0s K - which are basically 2 pions
              for( int j = 0; j < num; j++)
              {
                ...
                h_Ks_v0_daughter_pt->Fill(daug_pion->pt());
            ...
            if (match_KsV0_to_HPS && PF_hps_taus.isValid() )
            {
              ...
              for( unsigned tau_index = 0; tau_index < PF_hps_taus->size(); tau_index++)
              {
                ...
                //Check if tau jet kinematicaly matches to Ks of V0: tau^ks dr < 0.5
                  if (v_tau.DeltaR(v_Ks) < 0.5) // Ks are selected close to PV, so dR to tau jet ensures that the Ks of V0 is in tau jet
                  {
                    number_of_passed_ks_in_hps_tau_hard_cut++;
                    h_Ks_v0_found_in_hps_tau_dRcut->Fill(v_tau.DeltaR(v_Ks));
                  }
                  else continue;
                ...
                // Create pions collection of tau
                  ...
                  h_Ks_v0_n_pion_in_tau_jets_with_good_Ks->Fill(tau_picharge.size());
                ...
                //Pion to pion matching
                  ...
                  for(unsigned pi_i = 0; pi_i < tau_picharge.size(); pi_i++)
                  {
                    ...
                    h_Ks_v0_pions_and_hps_pions_combined_dR->Fill(tau_pion.DeltaR(v0_ks_pion1));
                    h_Ks_v0_pions_and_hps_pions_combined_dR->Fill(tau_pion.DeltaR(v0_ks_pion2));
                    ...
                    if (tau_picharge[pi_i]->charge() == (*V0Ks)[k0_i].daughter(0)->charge() &&
                        ...
                        (firstfound == -1 || deltaR1 > tau_pion.DeltaR(v0_ks_pion1)))// if matched to first pion of Ks V0 and it is matched better than the previous candidate
                    {
                ...
                // Invariant mass of two matched pions
                  if (firstfound != -1 && secondfound != -1)
                  {
                    ...
                    // //if (abs(inv_m_pions - 0.498) > 0.07) continue;
                    // number_of_passed_ks_in_hps_tau++;
                    // h_Ks_v0_found_in_hps_tau_m_inv->Fill(inv_m_pions);
                    // // dR between matched pions of tau and Ks V0
                    //   h_Ks_v0_found_in_hps_tau_dR->Fill(v_tau_picharge_firstfound.DeltaR(v_Ks));
                    //   h_Ks_v0_found_in_hps_tau_dR->Fill(v_tau_picharge_secondfound.DeltaR(v_Ks));

                     number_of_passed_ks_in_hps_tau++;
                     // dout("Is it different:", (TLorentzVector(tau_picharge[firstfound]->px(), tau_picharge[firstfound]->py(), tau_picharge[firstfound]->pz(), tau_picharge[firstfound]->energy())
                  ...
                  // dR between matched pions of tau and Ks V0
                    h_Ks_v0_found_in_hps_tau_dR->Fill(TLorentzVector(tau_picharge[firstfound]->px(), tau_picharge[firstfound]->py(), tau_picharge[firstfound]->pz(), tau_picharge[firstfound]->energy()).DeltaR(v_Ks));
                    h_Ks_v0_found_in_hps_tau_dR->Fill(TLorentzVector(tau_picharge[secondfound]->px(), tau_picharge[secondfound]->py(), tau_picharge[secondfound]->pz(), tau_picharge[secondfound]->energy()).DeltaR(v_Ks));

                  }
                  else if (firstfound == -1 || secondfound == -1) h_Ks_v0_found_in_hps_tau_dR_only_one_pion_left->Fill(v_tau.DeltaR(v_Ks));

              }
            }
        }
        // Fill the variables
          if (at_least_one_Ks_passed_dz) h_Ks_v0_n_ev_passing_dz_cut->Fill(1);
          //if (number_of_passed_ks_in_hps_tau_hard_cut>0)
            h_Ks_v0_n_Ks_in_jets_per_event->Fill(number_of_passed_ks_in_hps_tau_hard_cut);
          h_Ks_v0_count->Fill(v0_count);//h_v0_count->Fill(v0_count);
          h_Ks_v0_number_per_event->Fill(number_of_passed_ks);
          if (number_of_passed_ks_in_hps_tau > 0) h_Ks_v0_found_in_hps_tau->Fill(number_of_passed_ks_in_hps_tau);//"ks from V0 coll, NPE that passed PV and found in HPS tau jets"
      ...
      if (V0Ks_standart->size() > 0)
        for(unsigned k0_i = 0; k0_i < V0Ks_standart->size(); k0_i++) // over V0 K0s
        {
      ...
      if (false)
      {
        h_Ks_v0_n_tau_jets_per_event->Fill(PF_hps_taus->size());
        for( unsigned tau_index = 0; tau_index < PF_hps_taus->size(); tau_index++)
        {
          reco::PFTauRef pftauref(PF_hps_taus, tau_index);
          TLorentzVector v_tau(pftauref->px(), pftauref->py(), pftauref->pz(), pftauref->energy());
          double distMagXY = sqrt(pow(v_tau.X() - pv_position.x(),2) + pow(v_tau.Y() - pv_position.y(),2));
          h_tau_v0_dXY->Fill(distMagXY);
          ...
          vector < reco::PFCandidatePtr > tau_picharge = pftauref->signalPFChargedHadrCands(); // vector < edm::Ptr<PFCandidate> >
                                          tau_picharge.insert(tau_picharge.end(), tau_isolationPFChargedHadrCands.begin(), tau_isolationPFChargedHadrCands.end());
          h_Ks_v0_n_pion_in_tau_jets->Fill(tau_picharge.size());
          ...
          if (v0_count > 0)
          {
            for(unsigned k0_i = 0 ; k0_i < V0Ks->size() ; k0_i++)
            {
              ...
              for(unsigned pi_i = 0; pi_i < tau_picharge.size(); pi_i++)
              {
                ...
                h_Ks_v0_pions_and_hps_pions_combined_dR_with_no_constrain->Fill(tau_pion.DeltaR(v0_ks_pion1));
                h_Ks_v0_pions_and_hps_pions_combined_dR_with_no_constrain->Fill(tau_pion.DeltaR(v0_ks_pion2));
              }
            }
          }
