...
class hadd:
	...
	def __printInAndOut(self):
		...
		if not self.quiet:
			...
			print "Input files:"
			for f in self.inFiles:#in_files:
				print "\t", f
...
def main(argss=[__name__]):
	print "main args:", argss
	...
	(options, args) = parser.parse_args(argss)
	...
	print "going to send:", out_file, in_files, options.tmp_dir, options.verbose, options.vverbose, options.quiet, options.force_overwrite, options.save_tmp, options.nAtOnce, options.nJobs
	print
	## Set up and run hadd
